/**
 * Minimal Restate ingress client for command endpoints (0001:D2 verbs).
 *
 * Ingress URL shape (SPIKE-RESTATE.md (f)/0001:A4-5, Restate 1.7):
 *   POST {ingress}/{service}/{key}/{handler}        — request/response call
 *   POST {ingress}/{service}/{key}/{handler}/send   — one-way durable send
 *
 * DECISIONS 0001:A3/0001:A4 obligations implemented here:
 ...
 *
 * Spawn/message/control are all one-way durable sends (0001:D2: spawn = one-way
...
/** Build an ingress invocation URL. Throws IngressKeyError on an empty key (0001:A3). */
export function ingressUrl(
  ...
  if (target.key.length === 0) {
    throw new IngressKeyError(
      `empty Restate object key for service ${JSON.stringify(target.service)} (the gateway rejects empty keys)`,
    );
  }

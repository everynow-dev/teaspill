/**
 * Archive snapshot payload — the compact state D7 archives to the catalog and
 * T8.1 resurrection rehydrates from (shared contract between control.ts's
 * `applyArchive` writer and agent.ts's `resurrectFromCatalog` reader).
 *
 * The snapshot is the BOUNDED CONTEXT, not the timeline (D7): the K/V
 ...
 * remains the authoritative history (D1). The size bound below is enforced at
 * WRITE time (PLAN T8.1 anticipate: "enforce the bound at write time"), so an
 ...
 * cadence (A7): the bounded context should sit well under it, so the cap only
 * bites a pathological run. Comfortably below the outbox ~1 MiB per-event
 * journal budget (A4), so the `state_snapshot` stream event it also feeds never
...
export interface ArchiveSnapshotState {
  ...
  usage: RunUsage;
  /** Workspace key, or null (D4 — chosen at spawn, never switched). */
  workspaceRef: string | null;
  /** Parent entity url, or null. */
  parentRef: string | null;
  /** Subscriber entity urls (D2 pub/sub). */
  subscribers: string[];
  /** Opaque per-harness continuation state (D5), or null. */
...
/** Serialized-JSON byte size, the same measure the outbox uses at append time (R4). */
  ...
  if (serializedBytes(bounded) > maxBytes) {
    throw new ArchiveSnapshotTooLargeError(
      `archive snapshot exceeds ${maxBytes} bytes even with an empty context ` +
        ...
        `The archive snapshot must be the bounded context, not bulk data (D7/R4).`,
    );
  }
  return bounded;
}

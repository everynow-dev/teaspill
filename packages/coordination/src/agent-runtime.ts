/**
 * Agent virtual object — runtime context seam + K/V layout (0001:T2.1).
 *
 ...
 * against (the cron.ts `CronRuntimeCtx` pattern, extended with the 0001:A4
 ...
 * `restate.ObjectSharedContext` plus the 0001:A4 interrupt-race seam — so it can
 ...
 * conformance item (0001:T6.3/0001:T9.1), exactly as SPIKE-RESTATE.md prescribes for
...
// ---------------------------------------------------------------------------
// Entity status (0001:D7 lifecycle)
// ---------------------------------------------------------------------------

/**
 * 0001:D7: active → idle → archived.
 * - `active` — a wake (exclusive invocation) is being processed right now.
 * - `idle` — alive in Restate K/V, waiting for the next wake.
 * - `archived` — K/V cleared, snapshot in the catalog (0001:T8.1). Resurrection on
...
// ---------------------------------------------------------------------------
// K/V layout (0001:D1/0001:D2 — "Restate K/V = live entity state, the only store
// consulted for control flow")
// ---------------------------------------------------------------------------

/**
 * The complete K/V layout of an agent virtual object. Key names are the
 * literal Restate state keys. Layout per PLAN 0001:T2.1 / 0001:D2:
 ...
 * (`harness` — 0001:D5 layer-2 continuation state; `archiveEpoch` — the 0001:D7
 * idle-timer generation guard). Neither contradicts 0001:D2; both are live state
 * consulted only for control flow.
 */
export const AGENT_KV = {
  /** `EntityStatus` — 0001:D7 lifecycle position. Absent ⇒ never spawned. */
  status: "status",
  /**
   * `number` — the NEXT unallocated canonical seq (so `seq - 1` is the head
   * seq / last allocated). 0-based gapless per entity (0001:A1). Allocated ONLY by
   ...
   * event that consumes it (0001:D3). Harnesses/handlers never assign seq; they
   * produce `TimelineEventInit` and the outbox finalizes.
   */
  seq: "seq",
  /**
   * `TimelineEvent[]` — the pending projection outbox (0001:D3): events whose seq
   ...
   * unconfirmed) at the start of the next invocation. 0001:R4 budget: events are
   * staged and flushed in bounded chunks (see `commitEventsChunked`) so this
   * value stays well under the ~1 MiB journal-entry budget (0001:A4).
   */
  outbox: "outbox",
  /**
   * `TimelineEvent[]` — the bounded conversation context (0001:D1), in ascending
   ...
   * is fed from — never the stream (0001:D1: streams are never read for control
   * flow). Summarization is the bounding mechanism: a `summarization` event
   * drops everything it replaces from this value.
   */
  context: "context",
  /** `string` — workspace key `<tenant>/<name>` chosen at spawn (0001:D4: never switched). Absent when the agent has no workspace. */
  workspaceRef: "workspaceRef",
  /**
   * `string[]` — entity urls subscribed to this entity's changes (0001:D2
   * pub/sub). Notified via the notifier seam after each wake; subscription
   * management (subscribe/unsubscribe, debounce, dedupe) is 0001:T2.3.
   */
  subscribers: "subscribers",
  /** `string | null` — parent entity url (0001:D2 spawn carries the parent's key), null for root entities. */
  parentRef: "parentRef",
   ...
   * (0001:A4 interrupt target). Set first thing in every exclusive wake, cleared
   ...
   * (`HarnessStateDelta.harness`, 0001:D5): the CASDK harness stores
   * `{ sessionId, seqStamp }`, the native harness typically nothing.
   * Additive extension to the PLAN 0001:T2.1 key list (required by the frozen
   * harness interface).
   */
  harness: "harness",
  /**
   * `number` — generation guard for the idle→archive self-scheduled check
   * (0001:D7/0001:T8.1), same pattern as cron.ts: every wake bumps it and issues a
   ...
   * Additive extension to the PLAN 0001:T2.1 key list.
   */
  archiveEpoch: "archiveEpoch",
  /**
   * `boolean` — the 0001:T2.5 `pause`/`resume` runtime flag (0001:D8 control verb).
   ...
   * clears it and re-enqueues the mailbox. Deliberately SEPARATE from the 0001:D7
   ...
   * a paused entity's catalog status stays `idle` (0001:A5 freeze is not
   * touched). Additive extension to the PLAN 0001:T2.1 key list.
   */
  paused: "paused",
  /**
   ...
   * queued. Bounded by the same 0001:R4 budget as any K/V value; a pathological
   * flood while paused is the caller's concern (gateway backpressure, 0001:T1.2).
   */
...
// ---------------------------------------------------------------------------
// Interrupt error (0001:A4 seam)
// ---------------------------------------------------------------------------

/**
 * Thrown out of `AgentRuntimeCtx.raceInterrupt` when the in-flight wake is
 * cancelled by the shared `signal(interrupt)` handler (0001:T2.5's verb builds on
 ...
 * `explicitCancellation: true` (0001:A4) — can still perform durable cleanup
...
// ---------------------------------------------------------------------------
// Runtime contexts (structural subsets of the Restate contexts + 0001:A4 seam)
// ---------------------------------------------------------------------------
...
export interface AgentRuntimeCtx {
   ...
   * caller. Optional — fakes and pre-0001:T8.2 call sites omit it (span becomes a
   * normal root).
   */
  ...
  /**
   * Arm interrupt→abort WITHOUT racing/throwing (0001:T6.1 step-durable path).
   *
   * `raceInterrupt` is the 0001:T2.1 pattern for a harness that runs inside ONE
   ...
   * harness, 0001:T3.2) instead journals its OWN steps and treats an abort as a
   ...
   * (idempotent). Optional so the 0001:T2.1 fakes/handlers that never take the
 ...
 * one-way sends. This constraint is the design boundary 0001:T2.5 inherits.
 */

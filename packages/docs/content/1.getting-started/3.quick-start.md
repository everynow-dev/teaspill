...
::note
Live token deltas (`state.timeline.liveDeltas`) only flow when the deployment wires `emitDelta` into its `deps` — the minimal `serve()` above leaves it out, so `deltas: true` connects but stays quiet, and you'll still see every finalized message. [Building agents](/guides/agents/building-agents) shows where `emitDelta` goes.
::

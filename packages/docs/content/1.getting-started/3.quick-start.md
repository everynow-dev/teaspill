...
::note
You'll need the stack from [Installation](/getting-started/installation) running, the `teaspill` alias and `TEASPILL_API_KEY` from its last step set in your shell, and an Anthropic API key — the researcher calls a model.
::
...
`serve()` turns definitions into a running service and registers it through the gateway. Its `deps` are the deployment's connections into the stack — where events are written, and how agents notify each other:
...
The outbox writes each agent's [timeline](/concepts/timelines-events) to the history store and its row to the catalog; the `archiveCatalog` seam is what lets an idle agent archive and later come back — without it, an archived agent cannot be resurrected. The reference deployment under `packages/reference-deployment` wires these same seams; copy it when you outgrow one file.
...
By default each spawn mints a fresh, sortable id. Pass `--id my-researcher` to supply your own — re-running the same spawn then reattaches instead of duplicating, which is what makes scripted spawns safe to retry. [Multi-agent patterns](/concepts/multi-agent) builds on that trick.
...
The platform delivers your body to the agent as-is. If you'd rather accept shorthand like `{"text":"..."}`, normalize it at your deployment — the [Building agents guide](/guides/agents/building-agents) shows the pattern.

## Watch the timeline in a browser

You've already seen `teaspill logs` render the timeline in a terminal:
...
13:12:11  #3   ⚙ tool spawn_agent({"entityType":"summarizer","args":{"text":"…notes…"}})

...
An agent that stays idle past its idle window — 30 minutes by default, tunable per deployment with [`TEASPILL_IDLE_ARCHIVE_MS`](/reference/configuration#agent-loop-and-executor) — archives automatically: its complete state is stored in the [catalog](/concepts/projections-catalog), its working footprint is released, and its catalog status flips to `archived`. This is the default cost model — you never pay for agents that have nothing to do, and you never write cleanup code.

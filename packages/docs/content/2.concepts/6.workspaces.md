...
Pass a workspace name at spawn instead and you get a **shared** workspace: several agents pointing at the same name see the same filesystem. That's the tool for a team of agents collaborating on one checkout — spawn each with `workspace: "team-build"` and they read each other's files, still one command at a time.

...
::note
Choosing a workspace is agent-to-agent only: the `workspace` argument lives on the [`spawn_agent`](/guides/agents/building-agents) tool, so only an agent spawning another can set it. The gateway and CLI `spawn` take no workspace argument — an agent you spawn from outside always gets its own private workspace.
::

...
A **wake** is one turn of an agent's life. Something arrives — a message, a finished child, a timer — and the agent wakes: it does some work (maybe calls the model, maybe spawns children), then goes back to sleep. Agents never block or poll. Anything an agent is waiting for arrives as a later wake.

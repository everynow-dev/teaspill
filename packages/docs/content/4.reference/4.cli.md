...
The examples below (and elsewhere in these docs) call `teaspill` directly, assuming the shell alias from [Installation](/getting-started/installation) (`alias teaspill="node $PWD/packages/cli/dist/index.js"`). Without the alias, prefix any command with `pnpm --filter @teaspill/cli exec`.

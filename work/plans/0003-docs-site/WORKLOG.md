...
- **Status:** ▶ **ACTIVE — G4 done + committed. Wave 2 content COMPLETE** (G1–G4 landed). Full site builds: `generate` green, **115 routes, zero broken internal links**. Config resolved + applied: repo `https://github.com/everynow-dev/teaspill`, site `https://teaspill.everynow.dev`.
- **NEXT: dispatch G5** (T3.7 AI-surface & plumbing, **solo**; needs full page tree — now present; edits `nuxt.config.ts` + deletes template placeholders): remap `nuxt-llms` `title`/`description`/`sections` in `nuxt.config.ts` to OUR tree (Getting Started · Concepts · Guides · Reference · Contributing) — still shows template "Nuxt Docs Template" + Getting-Started/Essentials sections; **delete template placeholder dirs `content/2.essentials/` + `content/3.ai/`**; verify MCP `list-pages`/`get-page` + `/raw/*.md` against final tree. (site URL + `llms.domain` already set.)
- **Then G6** (T4.1 cutover + T4.3 CI/deploy — disjoint; **T4.1 needs 0002 fully done**): T4.1 = migrate/delete `docs/*.md` → stub (site + `work/` pointers), relocate not-migrated docs per ia.md table (`casdk-mapping.md`, `differences-from-electric-agents.md`, `self-hosting-networking.md` → `work/plans/0001-build-v1/notes/`), repo-wide dangling-`docs/`-ref sweep, **diff `docs/` vs baseline `c98d504`** to absorb any `0002:T5.4` fixes; also fix stale source headers found in G3/G4 (see below). T4.3 = CI job on **Node 22** (docs `generate`) + deploy options + `git remote add origin git@github.com:everynow-dev/teaspill.git` note.

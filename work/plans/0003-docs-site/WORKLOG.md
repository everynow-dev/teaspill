...
- **Status:** ⏸ **PAUSED at Gate 2** (INDEX flipped to `paused` 2026-07-19). G1+G2+G3 done + committed; **Wave 1 COMPLETE**; Gate 1 passed. **Blocked on 0002.** Checked 2026-07-19: 0002 is at G5 done / G6 next (T3.3 observability); `0002:T4.1` reference deployment **landed** ✅, but **`0002:T4.2` (live conformance) is still PENDING** — and `0002:T4.1` flagged 3 live-seam gaps (per-entity steer drain unwirable, `emitDelta` entityId gap, spawn `workspaceRef` unreachable) that T4.2 must confirm-live-or-fix, so runtime behavior Wave 2 documents is not yet stable. Gate 2 needs BOTH `0002:T4.1` landed AND `0002:T4.2` green → **not satisfied; Wave 2 NOT started.**
- **To RESUME (fresh session):** re-check `work/plans/0002-follow-ups/WORKLOG.md` resume pointer + `work/INDEX.md`. **When `0002:T4.2` is green** → flip 0003 back to `active` in INDEX and dispatch **G4** (Wave 2: T3.1 landing+getting-started [L] · T3.3 agent+frontend guides [M] · T3.4 ops guides [M] — parallel, disjoint content dirs; note the `content/3.guides/` subdir split + parent `.navigation.yml` owned by T3.3 only, per ia.md §3). Then G5 (T3.7 AI-surface + placeholder-content deletion, solo). **G6 (T4.1 cutover + T4.3 CI/deploy) additionally needs 0002 fully `done`** (its T5.4 sweep runs against old `docs/`). Then G7 (T4.2 QA, Gate 3). Wave-2 agents must read `notes/style-guide.md` + `notes/ia.md` + the "Notes for Wave-2 agents" below, and heed the 3 steer/delta/workspaceRef seam findings + verify every quick-start/guide command live against the current repo.

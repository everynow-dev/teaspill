...
- **Status:** ▶ **ACTIVE — G5 done + committed** (T3.7 AI-surface, done inline by main session — solo, no parallel race). G1–G5 landed. `generate` green, **115 routes, zero broken links**, docs lint/typecheck + root lint green.
- **NEXT: dispatch G6** (T4.1 cutover + T4.3 CI/deploy — disjoint; **T4.1 needs 0002 fully done** — per user, yes) (T4.1 cutover + T4.3 CI/deploy — disjoint; **T4.1 needs 0002 fully done**): T4.1 = migrate/delete `docs/*.md` → stub (site + `work/` pointers), relocate not-migrated docs per ia.md table (`casdk-mapping.md`, `differences-from-electric-agents.md`, `self-hosting-networking.md` → `work/plans/0001-build-v1/notes/`), repo-wide dangling-`docs/`-ref sweep, **diff `docs/` vs baseline `c98d504`** to absorb any `0002:T5.4` fixes; also fix stale source headers found in G3/G4 (see below). T4.3 = CI job on **Node 22** (docs `generate`) + deploy options + `git remote add origin git@github.com:everynow-dev/teaspill.git` note.

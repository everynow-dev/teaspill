...
- **Status:** ▶ **ACTIVE — G7/T4.2 QA done + committed. Gate 3 GREEN except the T4.1 sweep.** Only **T4.1 (docs/ cutover) remains** to close the plan — and it is HELD (see below). Everything in `packages/docs` is content-complete, audited, and building: `generate` green (115 routes, zero broken links), ban-list clean, docs lint/typecheck + root lint green, fresh-eyes findings all fixed.

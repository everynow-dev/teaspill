...
- **Status:** G1 + G2 + G3 done + committed. **Wave 1 COMPLETE.** Gate 1 passed. Next: **Gate 2 cross-plan wait** — check `work/plans/0002-follow-ups/` resume pointer + `work/INDEX.md`. Wave 2 (G4: T3.1 getting-started · T3.3 agent/frontend guides · T3.4 ops guides) needs `0002:T4.1` (reference deployment) landed AND `0002:T4.2` (live conformance) green. If 0002 hasn't reached those, **flip 0003 to `paused` in INDEX** with this pointer and STOP — do not author Wave 2 against a moving runtime. G6 (cutover T4.1) additionally needs 0002 fully `done`.

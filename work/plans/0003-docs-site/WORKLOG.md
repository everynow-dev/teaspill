...
- **Status:** ✅ **PLAN DONE (2026-07-19).** INDEX flipped 0003 → `done`. All 14 tasks complete: G1–G7 + T4.3 + the 0002-merge reconciliation + T4.1 cutover. The public docs site (`packages/docs`, `@teaspill/docs`) is the single source; `docs/` is retired to a stub. Final state green: docs `generate` 115 routes / zero broken links / docs lint+typecheck; root build+typecheck+lint green; ban-list clean. Deployable: static prerender, CI Node-22 docs job, `NUXT_PUBLIC_SITE_URL=https://teaspill.everynow.dev`.
- **Post-close follow-ups (not 0003):** (1) `ideas/agent-attention-status.md` — evaluate a catalog attention/awaiting-input field + typed tool (a future plan). (2) `TEASPILL_BACKUP_REREGISTER_CMD` / `BACKUP_LOSSY_RESTORE` — if they land in `scripts/{backup,restore}.sh`, add the re-registration note to the ops/backup-restore page. (3) stale SOURCE headers still saying "PROPOSED" (`schema/src/index.ts`, `harness-native/README.md`) — cosmetic source cleanup, left untouched (out of docs scope). (4) deploy: worktree has no git remote — `git remote add origin git@github.com:everynow-dev/teaspill.git` + pick a host (README documents static/Vercel).
...
## G6 — T4.1: Retire `docs/` — migration cutover ✅ (plan close)

Ran after the main-merge (0002 done, its T5.4 sweep already in `docs/`). Executed by an Opus subagent, verified + extended by the main session.
- **Relocated (git mv → `work/plans/0001-build-v1/notes/`)** the 3 internal-only artifacts: `casdk-mapping.md`, `differences-from-electric-agents.md`, `self-hosting-networking.md` (100% renames).
- **Deleted (git rm)** the 8 migrated guides: addressing, agents-sdk, auth, backup-restore, frontend-sdk, schema-reference, self-hosting, streams (content lives on the site + git history).
- **`docs/README.md` → stub** pointing at the site (`packages/docs`, dev command, https://teaspill.everynow.dev), `work/` + package READMEs for design history, and git history for the old guides. `docs/` now contains only `README.md`.
- **Repointed ~121 dangling `docs/*.md` refs** — migrated guides → site URLs (`https://teaspill.everynow.dev/...`), relocated files → `work/plans/0001-build-v1/notes/...`. Scope: package READMEs + `packages/*/src` comments + `AGENTS.md` + compose files + `references/` (agent), plus `scripts/{backup,restore}.sh` + `packages/harness-casdk/{Dockerfile,PACKAGING.md}` + `packages/reference-deployment/Dockerfile.executor` (main session — the agent flagged these as outside its enumerated scope).
- **Left untouched (correct):** all `work/plans/**` refs — append-only historical ledgers; rewriting them falsifies history. `CLAUDE.md` is a symlink to `AGENTS.md` (edited once).
- **Verified:** zero `docs/*.md` refs remain outside `work/plans` + the stub; root build/typecheck/lint green; docs generate 115 routes / zero broken links / docs typecheck+lint green.
